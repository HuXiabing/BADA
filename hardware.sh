...
BOARD_PASSWORD="starfive"           # Board password (optional)
...
Example:
    $0 --model_type transformer --train_data datasets/u74.json --epoch 5 --batch_size 16
EOF
    exit 0
}
...
log_info "=== Starting Automated Training Process ==="
...
while [[ $NO_IMPROVEMENT_COUNT -lt $NO_IMPROVEMENT_LIMIT ]]; do
    ...
    # Step 4: Run fuzzing
    log_step "4.$ITERATION Running bada/tool.py"
    GEN_CMD="python bada/tool.py --exp $CURRENT_EXPERIMENT_NAME --epoch $CURRENT_EPOCH --train $CURRENT_TRAIN_DATA_FILE --hardware"
    ...
    # File configuration
    INPUT_FILE="starfive_raw_data.json"              # File to send to board
    OUTPUT_FILE="result_$ITERATION.json"             # File returned from board
    OUTPUT_FILE1="result_$ITERATION.txt"
    BOARD_COMMAND="./run_test_streaming.sh $INPUT_FILE $OUTPUT_FILE > $OUTPUT_FILE1"       # Command to execute on board

    # Step 5: Send file to board
    log_step "5.$ITERATION Sending file to board"
    check_file "$INPUT_FILE"

    log_info "Sending $INPUT_FILE to board..."
    # Create working directory on board
    ssh ${BOARD_USER}@${BOARD_IP} "mkdir -p ${BOARD_WORK_DIR}"

    # Use scp to send file
    scp "$INPUT_FILE" ${BOARD_USER}@${BOARD_IP}:${BOARD_WORK_DIR}/

    log_info "File sent successfully"
    
    # Step 6: Execute command on board
    log_info "Step 6: Executing command on board..."

    ssh ${BOARD_USER}@${BOARD_IP} "cd ${BOARD_WORK_DIR} && ${BOARD_COMMAND}"

    log_info "Board execution completed"

    # Step 7: Retrieve result file from board
    log_info "Step 7: Retrieving result file from board..."

    scp ${BOARD_USER}@${BOARD_IP}:${BOARD_WORK_DIR}/${OUTPUT_FILE} ${LOCAL_WORK_DIR}/
    scp ${BOARD_USER}@${BOARD_IP}:${BOARD_WORK_DIR}/${OUTPUT_FILE1} ${LOCAL_WORK_DIR}/

    if [ ! -f "$OUTPUT_FILE" ]; then
        log_error "Failed to retrieve $OUTPUT_FILE from board!"
        exit 1
    fi

    log_info "Result file retrieved successfully"


    # Step 8: Run preprocess.py
    log_step "8.$ITERATION Running preprocess.py"
    ...
    PREPROCESS_CMD="python scripts/preprocess.py --cycle_jsons $OUTPUT_FILE --existing_train_json $CURRENT_TRAIN_DATA_FILE --existing_val_json $VAL_DATA --train_json $NEW_TRAIN_FILE --hardware"
    ...
    log_step "9.$ITERATION Running incremental training"
    ...
    # Step 10: Run inference on incremental training
    log_step "10.$ITERATION Running inference on incremental training (this may take a while...)"
